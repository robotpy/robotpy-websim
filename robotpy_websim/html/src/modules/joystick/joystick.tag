...
<joystick>
  ...
  <form class="form-horizontal" action="">
    <p class="axis" data-axis={axis.label} if={axis.visible} each={axis in opts.axes}>
      <slider label={axis.label} min="-1" max="1" val="{axis.value}" axis-index={axis.index} onchange={onSliderUpdate} />
    </p>
    <p class="pov" data-axis={pov.label} if={pov.visible} each={pov in opts.povs}>
      <slider label={pov.label} min="-1" max="360" val="{pov.value}" pov-index={pov.index} onchange={onPovUpdate} />
    </p>
    <div class="buttons">
